/**
 * Proton Drive SDK Service
 * HTTP wrapper bridge between Git LFS adapter and Proton Drive SDK
 * 
...
const app = express();
const PORT = process.env.SDK_SERVICE_PORT || 3000;
const backendMode = String(process.env.SDK_BACKEND_MODE || 'local').trim().toLowerCase();
const BACKEND_MODE_LOCAL = 'local';
const BACKEND_MODE_REAL = 'real';
const effectiveBackendMode = backendMode === BACKEND_MODE_REAL ? BACKEND_MODE_REAL : BACKEND_MODE_LOCAL;

if (backendMode !== effectiveBackendMode) {
  logger.warn(`Unknown SDK_BACKEND_MODE=${backendMode}; falling back to ${effectiveBackendMode}`);
}

function isRealBackendMode() {
  return effectiveBackendMode === BACKEND_MODE_REAL;
}
...
app.post('/init', async (req, res) => {
  try {
    ...
    if (isRealBackendMode()) {
      const resolvedDataPassword = dataPassword || process.env.PROTON_DATA_PASSWORD || password;
      const resolvedSecondFactorCode = secondFactorCode || process.env.PROTON_SECOND_FACTOR_CODE || '';
...
const server = app.listen(PORT, () => {
  logger.info(`Proton SDK Service v1.0.0 listening on port ${PORT}`);
  logger.info(`Backend mode: ${effectiveBackendMode}`);
